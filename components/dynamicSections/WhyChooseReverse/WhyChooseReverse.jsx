...
const WhyChooseReverse = ({ headline, upperHeadline, headlineSub, responsiveImgURL, renderedData, variant = 'primary' }) => {
        ...
        <div className='w-[360px] h-fit lg:w-[1200px] lg:min-h-[568px] bg-strongWhite lg:bg-transparent mx-auto lg:flex flex-col lg:gap-12'>
            ...
            <div className='w-full h-fit lg:flex hidden'>
                <div className='w-1/2 relative'>
                    <div className={`w-[680px] ${renderedData?.length > 5 && expandedIndex ? shaderHeight[renderedData?.length] : shaderHeight?.default} ${imgShader[variant]} absolute top-1/2 -translate-y-1/2 left-[140px] clip-path-polygon-reverse-four z-10 hidden lg:inline-block`}></div>
                    <div className='min-h-[500px] w-[790px] bg-strongWhite absolute top-1/2 -translate-y-1/2 left-0 clip-path-polygon-reverse-three z-50 hidden lg:inline-block'>
                        {renderedData?.map((item, index) => {
                            const mlValue = index * 10 + 0;
                            console.log(mlValue);
                            return (
                ...
                <div className='w-1/2'>
                    <div className='h-[438px] w-full'>
                        <Image src={responsiveImgURL ? responsiveImgURL : globalImages?.whyChooseSideImg} alt={headline ? headline : 'why-choose-techforing'} className='h-full w-full object-contain' />
                    </div>
                </div>
            </div>
            <div className='lg:hidden flex flex-col gap-6'>
                ...
                <div className='w-full h-[90px] relative'>
                    ...
                    <div className='w-[152px] h-[90px] absolute top-0 right-0 z-10 overflow-hidden'>
                        <Image src={responsiveImgURL ? responsiveImgURL : globalImages?.whyChooseSideImg} alt={headline ? headline : 'why-choose-techforing'} />
                    </div>
                </div>
                <div className='px-4'>
                    {renderedData?.map((item, index) => (
...
export default WhyChooseReverse;